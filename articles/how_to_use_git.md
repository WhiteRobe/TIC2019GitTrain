...
接下来，我们要说说如何真正操作Git完成版本地控制。先看下图：
...
5. 你会看到上图的反人类的`Vim`界面。(事实上可以通过修改配置文件来使用其它编辑器进行编辑commit信息，这部分内容请自行百度)在Windwos下的Git Bash的操作中，`Vim`的操作已经很和善了，每一次提交需要写一份提交信息，就像图中黄字所显示的那样，写作要点我已经用括号列出来了，现在讲讲这个界面如何进行确定、取消等操作。

![](/pic/VimBar.jpg)

6. 上图中你所看到的就是操作提示，以`^X Exit`为例，告诉你要使用`Ctrl+X`键来退出。由于退出时会询问你是否保存本次提交，因此此时你按下键盘上的`Ctrl+X`然后按下`Y`进行确认保存，此时还需要再按下一次`回车键`以最终提交。
...
仓库历史中你目前所需要关注的只有两个：橙框和绿框部分。绿框部分表示发生变动的**分支**，橙框部分是本次历史记录的ID。
...
3. 现在有两种`add`的操作，其一是点击工作区中文件前的图标对单个文件做`add`操作，或者点击`Stage Changed`按钮直接把工作区所有的文件进行`add`。接下来填写提交信息之后，点击`commit`进行提交操作。

> 同样进行`add`和`commit`的流程，使用Git GUI与使用Git Bash相比是不是前者更加直观明了？
...
现在，我们记得`test.txt`中的文字是【天气不好！】，现在我们要回滚回之前的那个版本，即【天气真好！】，要怎么做呢？
